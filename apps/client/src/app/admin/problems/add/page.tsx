...
export default function AddProblemPage() {
  ...
  const [isFetchingCF, setIsFetchingCF] = useState(false);
  ...
  const handleCFScrape = async () => {
    if (!formData.cfContestId || !formData.cfIndex) {
      alert('Please enter CF Contest ID and Index');
      return;
    }

    setIsFetchingCF(true);
    try {
      const res = await api.get(
        `/api/cf/problem?contestId=${formData.cfContestId}&index=${formData.cfIndex}`,
      );
      const data = res.data;
      ...
      setFormData((prev) => ({
        ...
        timeLimit: data.timeLimit?.toString() || prev.timeLimit,
        memoryLimit: data.memoryLimit?.toString() || prev.memoryLimit,
        statement: data.statement || prev.statement,
        testCases: data.testCases && data.testCases.length > 0 ? data.testCases : prev.testCases,
      }));
    } catch (err) {
      console.error('CF Scrape failed', err);
      alert('Failed to fetch from Codeforces. Problem may not exist or rate limited.');
    } finally {
      setIsFetchingCF(false);
    }
  };
    ...
    <ProtectedRoute>
      <div className="min-h-screen bg-bg-primary p-8 font-sans">
        <div className="max-w-4xl mx-auto flex flex-col gap-8">
          <h1 className="text-3xl font-bold tracking-tight text-text-primary">ADD PROBLEM</h1>
          ...
          >
            ...
            {formData.platform === 'CODEFORCES' && (
              <div className="bg-bg-elevated border border-blue/30 rounded-lg p-5 flex flex-col gap-4">
                <div className="flex items-center gap-2">
                  <span className="text-blue text-lg">ℹ</span>
                  <span className="text-sm font-mono text-blue">Codeforces Auto-Fill</span>
                </div>
                <div className="flex items-end gap-4">
                  <div className="flex flex-col gap-2 flex-1">
                    ...
                    <input
                      type="text"
                      value={formData.cfContestId}
                      onChange={(e) => setFormData({ ...formData, cfContestId: e.target.value })}
                      className="bg-bg-primary border border-border-default rounded px-3 py-2 text-sm text-text-primary focus:outline-none focus:border-blue"
                    />
                  </div>
                  <div className="flex flex-col gap-2 flex-1">
                    <label className="text-[10px] font-mono font-bold tracking-widest text-text-muted uppercase">
                    ...
                    <input
                      type="text"
                      value={formData.cfIndex}
                      onChange={(e) => setFormData({ ...formData, cfIndex: e.target.value })}
                      className="bg-bg-primary border border-border-default rounded px-3 py-2 text-sm text-text-primary focus:outline-none focus:border-blue"
                    />
                  </div>
                  <button
                    type="button"
                    onClick={handleCFScrape}
                    disabled={isFetchingCF}
                    className="h-[38px] px-4 rounded bg-blue/10 text-blue border border-blue/30 text-xs font-mono font-bold tracking-wider hover:bg-blue/20 transition-colors disabled:opacity-50"
                  >
                    {isFetchingCF ? 'FETCHING...' : 'FETCH FROM CF →'}
                  </button>
                </div>
              </div>
            )}
